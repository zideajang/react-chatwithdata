...
const CompareCountPlotComp = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${baseURL}/api/mcp/visualize/compare_countplot`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || "生成比较计数图失败");
      }
    ...
    <div className="box" >
      <div className="level">
        <div className="level-left">
          <figure className="image is-32x32">
            <img src="./images/chart.png" alt="compare count plot" /> {/* 替换为你的图标 */}
          </figure>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="field">
          ...
          <div className="control">
            <input
              className={`input ${errors.hue_column ? 'is-danger' : ''}`}
              type="text"
              id="hue_column"
              placeholder="请输入用于分组/着色的类别列名"
              {...register('hue_column', { required: 'Hue 列名是必需的' })}
            />
            {errors.hue_column && <p className="help is-danger">{errors.hue_column.message}</p>}
          </div>
        </div>
        ...
        {imageData && (
          <div>
            <h3 className="subtitle">生成的比较计数图</h3>
            <img src={`data:${imageData.mimeType};base64,${imageData.base64}`} alt="比较计数图" style={{ maxWidth: '100%', height: 'auto' }} />
          </div>
        )}
      </form>
    </div>
...
export default CompareCountPlotComp;